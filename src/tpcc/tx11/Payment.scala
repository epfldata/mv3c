 ...
 */
class Payment extends InMemoryTxImplViaMVCCTpccTableV4 with IPaymentInMem {
  ...
  override def paymentTx(datetime:Date, t_num: Int, w_id: Int, d_id: Int, c_by_name: Int, c_w_id: Int, c_d_id: Int, c_id: Int, c_last_input: String, h_amount: Float):Int = {
    ...
    try {
      ...
      val (w_name,w_street_1,w_street_2,w_city,w_state,w_zip,w_tax,w_ytd) = warehouse.row
      
      new ClosureTransition(List(warehousePred.onChange{ outputVersions:List[DeltaVersion[_,_]] =>
        val oldWarehouse = outputVersions(0).asInstanceOf[DeltaVersion[WarehouseTblKey,WarehouseTblValue]]
        val currentWarehouse = oldWarehouse.entry.getTheValue
        val w_ytd = currentWarehouse.row._8
        if(w_ytd != oldWarehouse.row._8) {
          List(warehouse.setEntryValue(currentWarehouse.row.copy(_8 = w_ytd+h_amount)))
        }
        Nil
      }), () => {
        (List(warehouse.setEntryValue(warehouse.row.copy(_8 = w_ytd+h_amount))),Nil)
      })

      val districtPred = ISharedData.tm.districtTbl.getPred((d_id,w_id))
      val district = districtPred.getEntry
      val (d_name,d_street_1,d_street_2,d_city,d_state,d_zip,d_tax,d_ytd,d_next_o_id) = district.row

      new ClosureTransition(List(districtPred.onChange{ outputVersions:List[DeltaVersion[_,_]] =>
        val oldDistrict = outputVersions(0).asInstanceOf[DeltaVersion[DistrictTblKey,DistrictTblValue]]
        val currentDistrict = oldDistrict.entry.getTheValue
        val d_ytd = currentDistrict.row._8
        if(d_ytd != oldDistrict.row._8) {
          List(district.setEntryValue(district.row.copy(_8 = d_ytd+h_amount)))
        }
        Nil
      }), () => {
        (List(district.setEntryValue(district.row.copy(_8 = d_ytd+h_amount))),Nil)
      })

      var customerPredicates = List[ChangeHandler[CustomerTblKey,CustomerTblValue]]()
      val customerPred = if (c_by_name > 0) {
        var customers = new ArrayBuffer[MiniCustomer]
        //we should slice over c_last_input
        val customerSlicePred = ISharedData.tm.customerTbl.slicePred(0, (c_d_id, c_w_id, c_last_input))
        val customerSlice = customerSlicePred.slice
        customerSlice.foreach { case ((c_id,_,_) , (c_first,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_)) =>
        ...
        customerPredicates = customerSlicePred.onChange{ outputVersions:List[DeltaVersion[_,_]] =>
          ...
          if(customers(index).cust_id != c_id) {
            val c_id = customers(index).cust_id
            //TODO make sure that the effects of the upcoming 
            //predicates are removed and they are executed with this new customer
            throw new RuntimeException("This case is not implemented yet, as there is not insertion/deletion from customer and their last names are immutable")
          }
          Nil
        } :: customerPredicates
      ...
      customerPredicates = customerPred.onChange { outputVersions:List[DeltaVersion[_,_]] =>
        Nil
      } :: customerPredicates
      ...
      new ClosureTransition(List(customerPred.onChange{ outputVersions:List[DeltaVersion[_,_]] =>
        val oldCustomer = outputVersions(0).asInstanceOf[DeltaVersion[CustomerTblKey,CustomerTblValue]]
        val currentCustomer = oldCustomer.entry.getTheValue
        val (c_first,c_middle,c_last,c_street_1,c_street_2,c_city,c_state,c_zip,c_phone,c_since,c_credit,c_credit_lim,c_discount,c_balance,c_ytd_payment,c_payment_cnt,c_delivery_cnt,found_c_data) = currentCustomer.row
        if(c_balance != oldCustomer.row._14 || c_credit != oldCustomer.row._11 || found_c_data != oldCustomer.row._18) {
          var c_data:String = found_c_data
          List(
          if (c_credit.contains("BC")) {
            ...
            currentCustomer.setEntryValue(currentCustomer.row.copy(_14 = c_balance+h_amount/*, _15 = c_ytd_payment+h_amount, _16 = c_payment_cnt+1*/, _18 = c_data))
          } else {
            //TODO this is the correct version but is not implemented in the correctness test
            currentCustomer.setEntryValue(currentCustomer.row.copy(_14 = c_balance+h_amount/*, _15 = c_ytd_payment+h_amount, _16 = c_payment_cnt+1*/))
          }
        )
        }
        Nil
      ...
      //TODO this is the correct version but is not implemented in the correctness test
      //TODO this is not inside a transition closure, as it can never create conflict, if found_c_id is not changed, but should be handled in general
      ...
      ISharedData.tm.historyTbl += ((found_c_id,c_d_id,c_w_id,d_id,w_id,ddbt.tpcc.loadtest.Util.roundDate(datetime),h_amount,h_data), TRUE_TUPLE)
