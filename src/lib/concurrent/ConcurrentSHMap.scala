...
object ConcurrentSHMap {
   ...
   */
  def comparableClassFor(x: Any): Class[_] = {
  ...
  def tabAt[K, V](tab: Array[Node[K, V]], i: Int): Node[K, V] = {
  ...
  final class TreeBin[K, V] extends Node[K, V](TREEBIN, null.asInstanceOf[K], null.asInstanceOf[V], null) {
     ...
     */
    final def putTreeVal(h: Int, k: K, v: V)(implicit ord: math.Ordering[K]): TreeNode[K, V] = {
  ...
  @SuppressWarnings(Array("serial")) final class ForEachEntryTask[K, V](p: BulkTask[K, V, _], b: Int, i: Int, f: Int, t: Array[Node[K, V]], final val action: Node[K, V] => Unit) extends BulkTask[K, V, Void](p,b,i,f,t) {
 ...
 */
class ConcurrentSHMap[K, V](projs:(K,V)=>_ *)(implicit ord: math.Ordering[K]) {
  /**
   ...
   */
  final def replaceNode(key: K, value: V, cv: Any): V = {
    val hash: Int = spread(key.hashCode)
  ...
  def forEach(parallelismThreshold: Long, action: (K,V) => Unit) {
    ...
    new ForEachEntryTask[K, V](null, batchFor(parallelismThreshold), 0, 0, table, { e => action(e.key, e.value) }).invoke
  }
  // @inline //inlining is disabled during development
  ...
  val idxs:List[ConcurrentSHIndex[_,K,V]] = {
    def idx[P](f:(K,V)=>P, lf:Float, initC:Int) = new ConcurrentSHIndex[P,K,V](f,lf,initC)(new Ordering[P] {
