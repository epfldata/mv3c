...
(
    i_id int NOT NULL PRIMARY KEY NONCLUSTERED HASH WITH (BUCKET_COUNT = 200000), 
...
(
    c_c_id smallint NOT NULL PRIMARY KEY NONCLUSTERED HASH WITH (BUCKET_COUNT = 2), 
...
INSERT INTO dbo.customer_credit_in_payment(c_c_id, c_c_credit) VALUES (1,'BC')
...
(
    ...
    */
    INDEX [fkey_history_2] NONCLUSTERED ([h_w_id], [h_d_id]) WITH ( BUCKET_COUNT = 1024)
) WITH (
  MEMORY_OPTIMIZED = ON, 
  DURABILITY = SCHEMA_ONLY
)
...
CREATE TYPE [dbo].[order_line_in_ostat] AS TABLE(
     ID int IDENTITY(1,1) PRIMARY KEY NONCLUSTERED HASH WITH (BUCKET_COUNT = 30),
...
(
    ...
    CONSTRAINT [PK_new_orders_no_w_id] PRIMARY KEY NONCLUSTERED ([no_w_id] ASC, [no_d_id] ASC, [no_o_id] ASC),
    /* NOT SUPPORTED BY IN-MEMORY DB
    , 
      ...
      (
        ...
        WHILE @loop_counter <= 10
            ...
            BEGIN
                ...
                SELECT @d_ol_total = CAST(sum(order_line.ol_amount) AS bigint)
                FROM dbo.order_line
                WHERE 
                    order_line.ol_o_id = @d_no_o_id AND 
                    order_line.ol_d_id = @d_d_id AND 
      ...
      (
        ...
        IF (@byname = 1)
            BEGIN
                ...
                /* 
                 *   Identifier MOD cannot be converted because it was not resolved.
        ...
        IF @found=1
            BEGIN
                ...
                SET @p_c_new_data = 
                    ...
                    /* 
                    *   function FORMAT is not supported in current SSMA version
                    */
