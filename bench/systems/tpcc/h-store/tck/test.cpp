 ...
 * REWORK IN PROGRESS:         *
 *******************************
 * Introducing packed struct instead of char* bullshit
 * Enabling costly and necessary condition branch (Payment)
 */
...
Heap<long>* lowest_no_id;

/*
#define PACKED __attribute__((__packed__))
#define ACCESSORS(tp,name,off) \
static inline void tp##_##name##_set(tp##_t* obj, const char* buf, int len) { int o=sizeof(tp##_t)+off; obj->_##name=o+len; memcpy(((char*)obj)+o, buf, len); } \
static inline void tp##_##name##_get(tp##_t* obj, char* buf) { int o=sizeof(tp##_t)+off; memcpy(buf,((char*)obj)+o, obj->_##name); ((char*)buf)[obj->_##name]=0; }

struct PACKED item_t {
  long id;
  long im_id;
  float price;
  char _name;
...
ACCESSORS(item,name,0)
ACCESSORS(item,data,obj->_name)
*/

#define SPRINT_TIMESTAMP(buf) { time_t now=time(NULL); tm *ltm=localtime(&now); snprintf((char*)(buf),11,"%04d-%02d-%02d:",ltm->tm_year+1900,ltm->tm_mon+1,ltm->tm_mday); }
...
int main(int argc, char** argv) {
  ...
  // FIXED:heap
  lowest_no_id = new Heap<long>[(count_ware+1)*10 + 1];
  ...
  if (!_core) {
    ...
    sw->start();
    // PROBLEM: increase here the number of transactions to run, 40K originally
    // FIXED: now 8M
    #ifndef NUM_LOOPS
    #define NUM_LOOPS 8000000
    #endif
    for (i=0; i< NUM_LOOPS /*40000*/; i++) {
  ...
  for (o_id=1; o_id<=ORD_PER_DIST; o_id++) {
    ...
    if (o_id > 2100) { /* the last 900 orders have not been delivered) */
       ...
       *((long*)(temp_tuple+offset)) = o_w_id;
       ...
       // FIXED:heap
       lowest_no_id[(int)(w_id*10 + d_id)].add(o_id);
...
int neword(long w_id, long d_id, long c_id, long ol_cnt,
  ...
  *((long*)(temp_tuple+offset)) = all_local;
  offset += sizeof(long);
  //TODO add timestamp here
  //PROBLEM: Avoiding the creation of output data
  //FIXED:
  SPRINT_TIMESTAMP(temp_tuple+offset)
  //memcpy(temp_tuple+offset, "2007-01-01:", 11);
...
read_inst();
  ...
  *((float*)(d_tuple + 2 * sizeof(long) + sizeof(float))) = d_ytd + (float)h_amount;
  ...
  //PROBLEM: fix for 'Disabling a costly and necessary condition branch (Payment)'
  //FIXED: enabled that branch
  if (strstr(c_credit, "BC")) {
    ...
    sprintf(c_new_data,"| %4ld %2ld %4ld %2ld %4ld $%7.2f",
                          (long)c_id, (long)c_d_id, (long)c_w_id, (long)d_id, (long)w_id, (double)h_amount);
...
void ordstat(long w_id, long d_id, long c_id,
  ...
  */
  ...
  if (byname) {
    ...
    // here we assume we are not going to have more than 300 duplicates
    //PROBLEM: Using fixed size structures to store unbounded relations (OrderStatus)
    //NOTE: no performance penalty: a realloc()'d array would have the same performance if inited with 300
    char* c_tuples[300];
  ...
  //PROBLEM: Using fixed size structures to store unbounded relations (OrderStatus)
  //NOTE: realloc()'d array would have same perf if starting with size=4000
  //FIXED: just increased from 100 -> 4000 to support larger load
...
void delivery(long w_id, long o_carrier_id) {
  ...
  //PROBLEM: Avoiding the creation of output data
  //FIXED:
  SPRINT_TIMESTAMP(ol_delivery_d)
  //memcpy(ol_delivery_d, "2007-01-01:", 11);
  ...
  for (long d_id=1; d_id<=DIST_PER_WARE; d_id++) {
    ...
    if (lowest_no_id[(int)(w_id*10+d_id)].empty()) {
    ...
    long o_id_low = lowest_no_id[(int)(w_id*10+d_id)].get();
    // FIXED:heap
    lowest_no_id[(int)(w_id*10+d_id)].remove(o_id_low);
    ...
    //PROBLEM: Replacing a minimum lookup query with a dummy value (Delivery)
  ...
  for (i=1; i<=20; i++) {
    ...
    for (l=1; l<=15; l++) {
      if (!orderline_tree.find((int)ORDER_LINEkey(o_id - i, d_id, w_id, long(l)),
        ...
        //PROBLEM: Simplifying a count-distinct into a traversal of the collection, without finding distinct elements (Stock-level)
        s_tuple = stockarray[(int)STOCKkey(ol_i_id, w_id)];

        slev_distinct.add(*((long*)s_tuple));
      }
    }
