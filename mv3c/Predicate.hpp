...
struct GetPred : public PRED {
    ...
    typedef TransactionReturnStatus(*FuncType)(Program *p,  DvPtr, ClosureState* cs);
    typedef ConcurrentStore<GetPred<K, V>> StoreType;
    ...
    forceinline DvPtr evaluateAndExecute(Transaction *xact, FuncType fn, ClosureState* cs = nullptr) {
        if (!isEvaluated) {
            isEvaluated = true;
            andThenFn = fn;
            closureState = cs;
...
struct PredicateBundle2 : PRED {
    ...
    PRED * const p2;
    bool flag1, flag2;
    typedef ConcurrentStore<PredicateBundle2> StoreType;
    static StoreType store;
...
struct PredicateBundle3 : PRED {
    ...
    typedef ConcurrentStore<PredicateBundle3> StoreType;
    static StoreType store;
